...
# Variables globales
OUTPUT_RESULTS    = "resultadosDemo.txt"             # Fichero donde se imprimen los resultados de las pruebas
...
def crear_archivos_con_entrevistas(data):
    ...
    while i in range(0,num_conv):
        ...
        i=i+1
    json_final={'lista':lista_historiales}
    with open("Historiales médicos dev.json", "w") as outfile: 
    ...
    while not fuera:
        ...
        # Obtener las preguntas del JSON y eliminar el número y el hashtag
            ...
            while temperatura<=MAX_TEMPERATURE:
                ...
                contexto=historial_medico+prompt
                ...
                for pregunta in preguntas_ordenadas:
                    ...
                    messages2="System: "+contexto+"User: "+pregunta
                    #modelo gpt 3.5-turbo
                    #response=get_completion(messages,temperature=float(temperatura))
                    #modelo instruct

                    response = get_completion_instruct(messages2, temperature=float(temperatura))
                    ...
                    if references[0]==['']:
                        references[0]="No se menciona en el historial medico."
                    ...
                    if predictions[0][-1]=="." and references[0][-1]==".":
                        predictions[0]=predictions[0][:-1]
                        references[0]=references[0][:-1]
                    elif predictions[0][-1]==".":
                        predictions[0]=predictions[0][:-1]
                    print("La pregunta es: ",pregunta)
                    print("Respuesta que debería decir",references[0])
                    print("Respuesta de ChatGPT",predictions[0])    
                    ...
                    if(references[0]=="No se menciona en el historial medico"):
                        
                        score_questions_cant_find_answers+=results["bleu"]
    
                        total_questions_cant_find_answers+=1
                    
                    elif(references[0]=="I I I I"):
                        
                        score_questions_no_need_answers+=results["bleu"]
